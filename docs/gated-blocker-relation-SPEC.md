...
### Notes rendering constraints (verified 2026-06-25)
Things' notes field renders a tiny Markdown subset: **bold, italic, strikethrough, named links `[label](url)` only**. NO headings (`#`), NO horizontal rules (`---`); the checklist field is separate plain text and cannot hold links. So the bold `**Gated by:**` / `**Gates:**` labels ARE the section structure. Always named links (`[Title](things:///show?id=…)`), never raw `things:///` URLs — named links render as clickable friendly text and sync across macOS/iOS.
...
2. On **dependent** notes: ensure a `**Gated by:**` managed block contains `[<blocker title>](things:///show?id=<blocker_uuid>)`. Add if absent; no-op if present (dedup by uuid).
3. On **blocker** notes: ensure a `**Gates:**` managed block contains `[<dependent title>](things:///show?id=<dependent_uuid>)`. Add if absent; no-op if present.
...
Recommended rendering (newlines render in notes, so multi-line is readable and matches the dev-issue):

```
**Gated by:**
[Reconfigure compositor VLAN200](things:///show?id=ABC123)
[Order the SFP module](things:///show?id=DEF456)
```

Parsing rule: locate the label line (`**Gated by:**` / `**Gates:**`), consume the contiguous following lines that match `[..](things:///show?id=<uuid>)` as the block's entries, dedup/add/remove by uuid, re-render, splice back into notes preserving any surrounding user text. Keep the managed block at a stable position (e.g. appended at end, separated by one blank line) so user-authored notes above it are never disturbed.

> Open decision (cosmetic, pick in build): multi-line block (above, more readable) vs. single comma-separated line (`**Gated by:** [A](…), [B](…)` — trivially parseable). Recommend multi-line; either is fine as long as parse/render are inverse and idempotent.
